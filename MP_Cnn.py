...
class CNN(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(3, 64, 3).to('cuda:4')
        self.conv2 = nn.Conv2d(64, 128, 3).to('cuda:4')
        self.conv3 = nn.Conv2d(128, 256, 3).to('cuda:5')
        self.pool = nn.MaxPool2d(2, 2)
        self.fc1 = nn.Linear(64 * 4 * 4, 128).to('cuda:4')
        self.fc2 = nn.Linear(128, 256).to('cuda:4')
        self.fc3 = nn.Linear(256, 10).to('cuda:5')

    def forward(self, x):
        x = self.pool(F.relu(self.conv1(x.to('cuda:4'))))
        x = self.pool(F.relu(self.conv2(x.to('cuda:4'))))
        x = self.pool(F.relu(self.conv3(x.to('cuda:5'))))
        x = x.view(-1, 64 * 4 * 4)
        x = F.relu(self.fc1(x.to('cuda:4')))
        x = F.relu(self.fc2(x.to('cuda:4')))
        x = self.fc3(x.to('cuda:5'))
...
def train(net, trainloader, optimizer, criterion, epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        inputs, targets = inputs, targets.to('cuda:5')
        optimizer.zero_grad()
        outputs = net(inputs)
